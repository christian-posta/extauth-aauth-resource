...
func TestParseAndVerifyAuthTokenRejectsMissingSubAndScope(t *testing.T) {
	...
	serverKey.Set(jwk.KeyIDKey, "ps-key-1")
	set := jwk.NewSet()
	...
	claims := map[string]interface{}{
		...
		"agent": "aauth:agent@agents.example.com",
		"iat":   time.Now().Unix(),
	...
	_, err = ParseAndVerifyAuthToken(token, set, "https://resource.example.com", false)
	if err == nil {
		t.Fatal("expected missing sub/scope error")
	}
}
